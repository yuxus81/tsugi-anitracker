/**
 * TSUGI V5 — Zeichensatz.
 *
 * Native Oberflaechen benutzen PAARE, keine Einheitsstriche:
 *   o = Kontur  -> inaktiv, ruhig, 2 px
 *   f = gefuellt -> aktiv, optisch schwerer, massive Flaeche
 *
 * Wo eine gefuellte Form Innenzeichnung braucht (Uhrzeiger, Haekchen im
 * Kreis), wird sie in --knock ausgestanzt: der Farbe des Traegers. Auf einer
 * farbigen Flaeche muss --knock diese Flaeche treffen, sonst klebt ein
 * dunkler Fleck im Symbol.
 *
 * Keine Emojis, keine Unicode-Zeichen — jedes Zeichen ist ein SVG-Pfad.
 * Uebernommen aus design-lab/v5-nativ/icons.js.
 */

interface IconDef {
  /** Kontur (inaktiv). */
  o: string;
  /** Fuellung (aktiv) — fehlt bei Zeichen, die nur als Kontur Sinn ergeben. */
  f?: string;
}
...
export function iconPath(name: IconName, filled: boolean): { d: string; solid: boolean } {
  const def: IconDef = PATHS[name];
  // `filled` ist ein Wunsch, kein Versprechen: Zeichen ohne massive Fassung
  // liefern weiter ihre Kontur. Ein leeres SVG waere schlimmer.
  const solid = filled && typeof def.f === 'string';
  return { d: solid ? (def.f as string) : def.o, solid };
}
